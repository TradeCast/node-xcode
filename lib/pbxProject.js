...
pbxProject.prototype.addResourceFile = function(path, opt, group) {
    ...
    file.uuid = this.generateUuid();
    file.target = opt ? opt.target : undefined;
    var hasFileInReference = this.hasFile(path);
    ...
    if (opt.plugin) {
        ...
        var hasFileInTargetBuildPath = file.target ? this.hasInTargetBuildPath(file) : false;

        // not new file, try to add to target
        if (!hasFileInReference || (hasFileInReference && !hasFileInTargetBuildPath)) {

            if (!opt.plugin) {
                if (!hasFileInReference) {
                    this.addToPbxFileReferenceSection(file);    // PBXFileReference
                }

                var fileInPbxBuildFile = this.isInPbxBuildFileSection(file); //PBXBuildFile

                if (fileInPbxBuildFile) {
                    file.uuid = fileInPbxBuildFile = fileInPbxBuildFile.uuid;
                } else {
                    this.addToPbxBuildFileSection(file);        // PBXBuildFile
                }
...
/**
 * Find file ref in PbxBuildFileSection
 * @param file
 * @returns {String} file uuid
 */
pbxProject.prototype.isInPbxBuildFileSection = function(file) {
    var foundFileRef;
    var uuid;

    for (uuid in this.pbxBuildFileSection()) {
        if (this.pbxBuildFileSection()[uuid].fileRef_comment == file.basename) {
            foundFileRef = this.pbxBuildFileSection()[uuid];
            foundFileRef.uuid = uuid;
        }
    }

    // return file of found file in PbxBuildFileSection
    return foundFileRef;
}
...
// check if file is already in target build path
pbxProject.prototype.hasInTargetBuildPath = function(file) {
    var sources = this.pbxResourcesBuildPhaseObj(file.target);

    var hasAlreadyInFiles = false;

    for (i in sources.files) {
        if (sources.files[i].comment == longComment(file)) {
            hasAlreadyInFiles = true;
            break;
        }
    }

    return hasAlreadyInFiles;
};
...
// get file id
pbxProject.prototype.getFileId = function(filePath) {
    var files = nonComments(this.pbxFileReferenceSection()),
        file, id;
